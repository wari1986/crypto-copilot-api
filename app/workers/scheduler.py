...
from app.db.session import async_session as default_session_maker
...
async def start_market_data_tasks(session_factory: async_sessionmaker[AsyncSession] | None = None) -> None:
    session_factory = session_factory or default_session_maker
    cache = MarketCache()
    ...
    # Backfill OHLCV in the background (simple serial loop for now)
    ...
    asyncio.create_task(ws.start_orderbook(settings.symbols_list, settings.ws_orderbook_levels, session_factory))
